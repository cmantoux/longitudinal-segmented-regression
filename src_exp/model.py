...
within the framework of the exponentialized model.
...
def get_prior(K, d, exp_variance=0.1, regularize_xi=6.):
    """
    K: number of breakpoints
    d: data dimension
    exp_variance: variance of the exponential model. A variance close to zero
                  reduces the distance to the base model but slows the SAEM convergence.
    ...
    Returns a standard prior for the exponentialized model.
    """

    sigma_p0 = exp_variance
    sigma_t0 = exp_variance
    sigma_v0 = exp_variance
    p0_barbar = np.zeros(d)
    t0_barbar = np.arange(max(K, 1))
    v0_barbar = np.zeros((K+1, d))
    sigma_p0_bar = 500.
    sigma_t0_bar = 500.
    sigma_v0_bar = 500.
    m, v = ([float(regularize_xi), 6., 6., 6.], [1., 1., 1., 1.]) # inverse gamma prior for the variances of sigma_xi, sigma_tau, sigma_psi, sigma (in this order)
    return (sigma_p0,sigma_t0,sigma_v0,p0_barbar,t0_barbar,v0_barbar,
...
def pack_theta(theta):
    ...
    return np.concatenate((p0_bar, t0_bar, v0_bar.reshape(-1), sigma_xi, sigma_tau, sigma_psi, sigma))
...
def unpack_theta(theta_vect, d, K):
    ...
    p0_bar, theta_ = theta_vect[:d], theta_vect[d:]
    t0_bar, theta_ = theta_[:max(K,1)], theta_[max(K,1):]
    v0_bar, theta_ = theta_[:(K+1)*d].reshape((K+1, d)), theta_[(K+1)*d:]
    sigma_xi  = theta_[:K+1]
    sigma_tau = np.array([theta_[K+1]])
    sigma_psi = theta_[K+2:K+d+2]
    sigma     = theta_[K+d+2:]
    return p0_bar, t0_bar, v0_bar, sigma_xi, sigma_tau, sigma_psi, sigma
...
def pack_z(z):
    ...
    z_vect = np.concatenate((p0, t0, v0.reshape(-1), tau, xi.reshape(-1), psi.reshape(-1))) # d + K + (K+1)*d + N + N + N*d = K+d*(K+2)+(K+2+d)*N
    return z_vect
...
def unpack_z(z_vect, d, K):
    ...
    N = int((len(z_vect)-(K+d*(K+2)))/(K+d+2))
    p0 = z_vect[:d]
    t0 = z_vect[d:d+max(K, 1)]
    v0 = z_vect[d+max(K, 1):d*(K+1+1)+max(K, 1)].reshape(K+1, d)
    tau = z_vect[d*(K+2)+max(K, 1):d*(K+2)+max(K, 1)+N]
    xi = z_vect[d*(K+2)+max(K, 1)+N:d*(K+2)+max(K, 1)+(K+2)*N].reshape(N, K+1)
    psi = z_vect[d*(K+2)+max(K, 1)+(K+2)*N:d*(K+2)+max(K, 1)+(K+2+d)*N].reshape(N, d)
    return (p0, t0, v0, tau, xi, psi)
...
def get_indices(i, N, d, K):
    ...
    offset = d+max(K, 1)+d*(K+1) # length of (p0, t0, v0) in z_vect
    return np.array((
        (offset+i, offset+i+1), # tau_i
        (offset+N+(K+1)*i, offset+N+(K+1)*(i+1)), # xi_i
        (offset+N*(K+2)+d*i, offset+N*(K+2)+d*(i+1)) # psi_i
    ))
...
def unpack_ind(z_vect, i, d, K):
    ...
    N = int((len(z_vect)-(max(K, 1)+d*(K+2)))/(K+d+2))
...
@njit
def time_reparam(taus, xi, time, t0):
    """
    ...
    K = len(xi) - 1
    if K==0:
        return t0[0] + np.exp(xi[0]) * (time - t0[0] - taus[0])
    else:
        k = src_common.utils.digitize(time, t0+taus[1:])
        if k==0:
            return t0[0] + np.exp(xi[0]) * (time - t0[0] - taus[0])
        else:
            return t0[k-1] + np.exp(xi[k]) * (time - t0[k-1] - taus[k])
...
def D_i_alt(ps0, t0, v0, z_i_full, time):
    ...
    taus, xi, psi = z_i_full
    return D(ps0, t0, v0, time_reparam(taus, xi, time, t0)) + psi
...
def occupation_rates_ind(t, z, theta, rnd=3):
    ...
    for i in range(N):
        z_tmp = None, None, None, [tau[i]], [xi[i]], None
...
@njit
def log_lk_split(y_mat, t_mat, n, z_vect, theta, prior, orthogonality_condition=False):
    """
    ...
    Computes log p(y, z | theta), split into N individual components.
    """

    p0_bar, t0_bar, v0_bar, sigma_xi, sigma_tau, sigma_psi, sigma = theta
    sigma_p0,sigma_t0,sigma_v0,p0_barbar,t0_barbar,v0_barbar = prior[:6]
    sigma_p0_bar,sigma_t0_bar,sigma_v0_bar,m,v = prior[6:]
    ...
    # res[-1] contains the terms for population parameters
    res[-1] -= 0.5*np.sum(p0-p0_bar)**2/sigma_p0**2         + d*(np.log(sigma_p0) + c)
    res[-1] -= 0.5*np.sum(t0-t0_bar)**2/sigma_t0**2         + max(K, 1)*(np.log(sigma_t0) + c)
    res[-1] -= 0.5*np.linalg.norm(v0-v0_bar)**2/sigma_v0**2 + (d*(K+1))*(np.log(sigma_v0) + c)
    return res


@njit
def log_lk(y_mat, t_mat, n, z_vect, theta, prior, orthogonality_condition=False):
    """
    ...
    return log_lk_split(y_mat, t_mat, n, z_vect, theta, prior, orthogonality_condition).sum()
...
def log_lk_ind(y_mat, t_mat, n, z_vect, theta_full, i, orthogonality_condition=False):
    ...
    Computes log p(y_i, z_i | theta) for a given individual.
    """

    ps0, t0_bar, v0_bar, sigma_xi, sigma_tau, sigma_psi, sigma = theta_full
...
def log_lk_tau_xi_ind(y_mat, t_mat, n, z_vect, theta_full, i):
    ...
    Computes log p(tau_i, xi_i | y_i, theta) for a given individual
...
def log_lk_full(y_mat, t_mat, n, z_vect, theta, prior, orthogonality_condition=False):
    ...
    res -= 0.5*np.linalg.norm(p0-p0_bar)**2/sigma_p0**2 + d*(np.log(sigma_p0) + c)
    res -= 0.5*np.linalg.norm(t0-t0_bar)**2/sigma_t0**2 + max(K, 1)*(np.log(sigma_t0) + c)
    res -= 0.5*np.linalg.norm(v0-v0_bar)**2/sigma_v0**2 + (d*(K+1))*(np.log(sigma_v0) + c)

    res -= 0.5*np.linalg.norm(p0_bar-p0_barbar)**2/sigma_p0_bar**2
    res -= 0.5*np.linalg.norm(t0_bar-t0_barbar)**2/sigma_t0_bar**2
    res -= 0.5*np.linalg.norm(v0_bar-v0_barbar)**2/sigma_v0_bar**2

    res -= v[0]**2/(2*sigma_xi**2).sum()  + (m[0]+2)*np.log(sigma_xi).sum()
    res -= v[1]**2/(2*sigma_tau[0]**2)    + (m[1]+2)*np.log(sigma_tau)[0]
    res -= v[2]**2/(2*sigma_psi**2).sum() + (m[2]+2)*np.log(sigma_psi).sum()
